...
$app->add(function ($request, $response, $next) {
    ...
    if (!in_array($request->getUri()->getPath(), ['about', 'login', 'callback'])) {
        ...
        }else if(!empty($status->user) && isset($status->application->app_url) && strpos($status->application->app_url, 'inscription_gala') === false){ // il était connecté en tant qu'icam mais l'appli non
            try {
